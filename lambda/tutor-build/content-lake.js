...
function _enforceSaveSchema(candidate, contextForLog) {
  ...
  if (t === 'multiple_choice') {
    if (!Array.isArray(candidate.choices) || candidate.choices.length < 2) {
      ...
      // §110 widget support: choices may be strings OR widget-spec objects.
      // For object choices, validate the widget spec server-side (mirror of
      // the frontend's GradeEarnWidgets.validate). Plain strings still
      // pass through unchanged — every existing text-only question keeps
      ...
      for (let i = 0; i < candidate.choices.length; i++) {
        ...
        if (typeof c === 'string') {
          ...
          const wErr = _validateWidgetSpec(c);
    ...
    // Optional stimulus widget on the question itself.
    if (candidate.stimulus != null) {
      if (typeof candidate.stimulus !== 'object' || !candidate.stimulus.type) {
        ...
        const sErr = _validateWidgetSpec(candidate.stimulus);
...
/*
 * §110 widget-spec validator — server-side mirror of the per-widget
 * __validate functions in js/widgets/*.js. Returns a short error
 * string ('parts_out_of_range', etc.) or null if the spec is valid.
 *
 * KEEP IN SYNC with js/widgets/<type>.js validators. The frontend +
 * lambda must agree on what's a valid spec, otherwise the lake will
 * accept content that the kid's browser can't render.
 */
function _validateWidgetSpec(spec) {
