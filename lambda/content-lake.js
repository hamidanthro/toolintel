...
// ============================================================
// §110 widget-spec server-side validators — extracted to
// lambda/widget-validators.js so the offline test scripts (which
// don't have @aws-sdk installed) can require the validator without
// booting the full lake module. KEEP THIS FILE IN SYNC with the
// frontend per-widget __validate functions in js/widgets/<type>.js.
const { validateSaveSchema: _runSaveSchema } = require('./widget-validators');

function _enforceSaveSchema(candidate, contextForLog) {
  const errors = _runSaveSchema(candidate);
  if (errors.length) {
